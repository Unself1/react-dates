  ...
  _createClass(CalendarMonthGrid, [{
    ...
    value: function () {
      function componentWillReceiveProps(nextProps) {
        ...
        var hasYearChanged = this.props.initialMonth.clone().add(1, 'year').isSame(initialMonth) || this.props.initialMonth.clone().subtract(1, 'year').isSame(initialMonth);
        var hasNumberOfMonthsChanged = this.props.numberOfMonths !== numberOfMonths;
        var hasResetToToday = nextProps.initialMonth.isSame((0, _moment2['default'])(), 'month');
        var newMonths = months;

        if (hasYearChanged) {
          if (this.props.initialMonth.clone().add(1, 'year').isSame(initialMonth)) {
            newMonths = months.map(function (month) {
              return month.add(1, 'year');
            });
          } else {
            newMonths = months.map(function (month) {
              return month.subtract(1, 'year');
            });
          }
